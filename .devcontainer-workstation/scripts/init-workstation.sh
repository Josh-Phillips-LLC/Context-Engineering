...
RUNTIME_AGENTS_ADAPTER_FILE="${RUNTIME_AGENTS_ADAPTER_FILE:-AGENTS.md}"
RUNTIME_COPILOT_INSTRUCTIONS_FILE="${RUNTIME_COPILOT_INSTRUCTIONS_FILE:-.github/copilot-instructions.md}"
RUNTIME_VSCODE_SETTINGS_FILE="${RUNTIME_VSCODE_SETTINGS_FILE:-.vscode/settings.json}"
...
render_instruction_adapter_files() {
  local target_file="${WORKSPACE_REPO_DIR}/${RUNTIME_ROLE_INSTRUCTIONS_FILE}"
  local agents_file="${WORKSPACE_REPO_DIR}/${RUNTIME_AGENTS_ADAPTER_FILE}"
  local copilot_file="${WORKSPACE_REPO_DIR}/${RUNTIME_COPILOT_INSTRUCTIONS_FILE}"
...

  {
    ...
    echo "Generated by .devcontainer-workstation/scripts/init-workstation.sh"
    echo
    echo "Use the canonical runtime role instructions from the workspace root:"
    echo
    echo "- [Runtime role instructions](../${RUNTIME_ROLE_INSTRUCTIONS_FILE})"
    echo "- Role profile: ${ROLE_PROFILE}"
    echo
    echo "Do not reinterpret or override role authority boundaries defined there."
  } > "$copilot_file"

  chmod 644 "$agents_file" "$copilot_file"
...
ensure_workspace_vscode_settings() {
  local settings_file="${WORKSPACE_REPO_DIR}/${RUNTIME_VSCODE_SETTINGS_FILE}"
  local settings_dir
  local defaults_json
  ...
  echo "Ensured workspace VS Code chat settings at ${settings_file}."
}
